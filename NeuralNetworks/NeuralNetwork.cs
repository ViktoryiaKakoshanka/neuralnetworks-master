...
namespace NeuralNetworks
{
    public class NeuralNetwork
    {
        ...
        public List<Neuron> PredictTest(params double[] inputSignals)
        {
            SendSignalsToInputNeurons(inputSignals);
            FeedForwardAllLayersAfterInput();
        ...
        public double Learn(double[] expected, int[,] inputs, int epoch)
        {
            var error = 0.0;
        ...
        public double[] LearnTest(List<double[]> expected, List<double[]> inputs, int epoch)
        {
            var errors = new[] { 0.0 , 0.0 , 0.0 };
            for (var i = 0; i < epoch; i++)
            {
                for (var k = 0; k < expected.Count; k++)
                {
                    ...
                    var input = inputs[k];
                    var errorList = BackpropagationTest(output, input);
                    for (var j = 0; j < errorList.Length; j++)
                    {
                        errors[j] += errorList[j];
                    }
                }
            }
            ...
            for (var i = 0; i < 3; i++)
            {
                result[i] = errors[i] / epoch;
            }
            
            return result;
        }

        public static double[] GetRow(int[,] matrix, int row)
        {
        ...
        private double[] BackpropagationTest(double[] exprecteds, params double[] inputs)
        {
            var actuals = PredictTest(inputs);
            var differences = new double[3];
            for (var i = 0; i< actuals.Count; i++)
            {
                differences[i] = actuals[i].Output - exprecteds[i];
            }
            ...
            foreach (var neuron in Layers.Last().Neurons)
            {
                neuron.Learn(differences[a], Topology.LearningRate);
                a++;
            }
            ...
            for (var i = 0; i < actuals.Count; i++)
            {
                result[i] = differences[i] * differences[i];
            }
            return result;
        }
